...
<body>
    ...
    <div class="header">
        ...
        <h2>Slip Gaji Pegawai (Self-Service)</h2>
    </div>
    ...
    @if(isset($kurirSalary) && $kurirSalary->count() > 0)
        <div class="period-info">
            ...
            @if($gajiSettings)
                <p><strong>Tarif Saat Ini:</strong> Paket Bawaan Rp {{ number_format($gajiSettings->paket_bawaan, 0, ',', '.') }}, 
    ...
    @if((!isset($traditionalSalary) || count($traditionalSalary) == 0) && (!isset($kurirSalary) || $kurirSalary->count() == 0))
        <div class="no-data">
            ...
            <p>Silakan periksa periode lain atau hubungi administrator jika ada masalah.</p>
        </div>
    @endif
    ...
    <div style="margin-top: 30px; padding: 15px; background-color: #fff3cd; border: 1px solid #ffeaa7; border-radius: 5px;">
        ...
        <ul>
            <li>Slip gaji ini dicetak secara otomatis melalui sistem self-service</li>
            <li>Jika ada perbedaan data atau kesalahan, silakan hubungi HRD</li>
            <li>Slip gaji ini berlaku sebagai bukti pembayaran resmi</li>
            <li>Dokumen ini dicetak pada {{ date('d M Y H:i:s') }}</li>
        </ul>
    </div>
    ...
    <div class="signature">
        <div class="signature-box">
            ...
            <p style="font-size: 0.9em;">{{ date('d M Y') }}</p>
        </div>
        <div class="signature-box">
            <p><strong>PT. Indonesia Damai</strong></p>
            <p>Sistem Informasi Gaji</p>
            <br><br><br>
            <p>_____________________</p>
            <p><strong>Authorized by System</strong></p>
        </div>
    </div>
